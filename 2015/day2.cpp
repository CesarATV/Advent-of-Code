...
#define PUZZLE_INPUT_FILE_NAME "puzzleInputs/day2.txt"
#define PUZZLE_EXAMPLE_INPUT_FILE_NAME "puzzleInputs/day2_example.txt"

#define DELIMITER_CHARACTER 'x'


void first_part(std::vector<std::string> const &lines) 
{
    ...
    for (std::string line : lines)
    {
        ...
        size_t next_delimiter = line.find(DELIMITER_CHARACTER);
        ...
        uint slack_area = *std::min_element(areas_array.begin(), areas_array.end());
...
void second_part(std::vector<std::string> const &lines) 
{
    uint total_ribons = 0;
    ...
    {     
        ...
        uint volume = std::accumulate(sides_array.begin(), sides_array.end(), 1, std::multiplies<>());

...
#define PUZZLE_INPUT_FILE_NAME "puzzleInputs/day1.txt"
#define PUZZLE_EXAMPLE_INPUT_FILE_NAME "puzzleInputs/day1_example.txt"

#define FLOOR_UP_CHARACTER '('
#define FLOOR_DOWN_CHARACTER ')'


void first_part(const std::string &file_contents) 
{
    int n_floors_up = std::count(file_contents.begin(), file_contents.end(), FLOOR_UP_CHARACTER);
    int n_floors_down = std::count(file_contents.begin(), file_contents.end(), FLOOR_DOWN_CHARACTER);
...
void second_part(const std::string &file_contents) 
{
    int current_floor = 0;

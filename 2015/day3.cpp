...
#define PUZZLE_INPUT_FILE_NAME "puzzleInputs/day3.txt"
#define PUZZLE_EXAMPLE_INPUT_FILE_NAME "puzzleInputs/day3_example.txt"
...
void first_part(const std::string &file_contents) 
{
    ...
    for (char direction_character : file_contents)
    {
    ...
    uint n_unique_visited_houses = visited_houses.size();
...
void second_part(const std::string &file_contents) 
{
    ...
    for (uint file_content_idx=0, santa_idx=0; file_content_idx < file_contents.size(); file_content_idx++, santa_idx++ )
    {
        ...
        char direction_character = file_contents.at(file_content_idx);
        switch (direction_character)
        {
            case NORTH_CHARACTER:

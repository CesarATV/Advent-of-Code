...
#define PUZZLE_INPUT_FILE_NAME "puzzleInputs/day5.txt"
#define PUZZLE_EXAMPLE_INPUT_FILE_NAME "puzzleInputs/day5_example.txt"


void first_part(std::ifstream &puzzle_file) 
{
    const std::regex regex_contains_enough_vowells("(.*[aeiou]){3,}"); // ./ allows matches even when the letters are not together, [aeiou] matches any latin vowel and {3,} does so that the previous match has to appear three times or more 
    ...
    {
        if (std::regex_search(current_line.begin(), current_line.end(), regex_contains_enough_vowells) == true && std::regex_search(current_line.begin(), current_line.end(), regex_repeated_letters_in_a_row) == true && std::regex_search(current_line.begin(), current_line.end(), regex_uncontainable_strings) == false)
        {
...
void second_part(std::ifstream &puzzle_file) 
{
    ...
    while(std::getline(puzzle_file, current_line))
    {
        if (std::regex_search(current_line.begin(), current_line.end(), regex_twice_pair_of_two_letters) == true && std::regex_search(current_line.begin(), current_line.end(), regex_repeated_letter_with_one_between_them) == true)
        {

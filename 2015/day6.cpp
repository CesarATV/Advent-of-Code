...
#define PUZZLE_INPUT_FILE_NAME "puzzleInputs/day6.txt"
#define PUZZLE_EXAMPLE_INPUT_FILE_NAME "puzzleInputs/day6_example.txt"

#define N_ROWS 1000
#define N_COLS 1000


enum LightInstruction { TurnOn, TurnOff, Switch };
...
void parse_puzzle_line(LightInstruction *light_instruction, uint *beginning_row, uint *beginning_col, uint *ending_row, uint *ending_col, std::string const &line)
{
    ...
    size_t next_delimiter = line.find(" ");
...
void first_part(std::vector<std::string> const &lines) 
{
    ...
    for ( std::pair<uint, uint> light_position : light_grid_indexes_yet_to_process)
    {
...
void second_part(std::vector<std::string> const &lines)
{
    ...
    for ( uint light_brightness : light_grid_brightness)
    {

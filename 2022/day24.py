...
PUZZLE_INPUT_FILE_NAME = "puzzleInputs/day24.txt"
PUZZLE_EXAMPLE_INPUT_FILE_NAME = "puzzleInputs/day24_example.txt"
...
def get_blizzard_map_over_time(starting_position, ending_position, n_rows, n_cols, left_blizzards, right_blizzards, up_blizzards, down_blizzards):
    ...
    covered_by_blizzard_map[:,:2,:] = True
    covered_by_blizzard_map[:,n_rows-2:,:] = True 
    ...
    for time_idx in range(time_until_blizzards_loop):
        ...
        up_blizzards[up_blizzards[:,0] <= up_limit] += [down_limit-2,0]
        down_blizzards[down_blizzards[:,0] >= down_limit] -= [down_limit-2,0] - np.array([up_limit-1,0])
...
def consider_movement(paths_to_visit, paths_considered_to_visit, covered_by_blizzard_map, current_time_mod, considered_time, considered_position):
    considered_position_mod = str([current_time_mod,considered_position[1], considered_position[2]])
    '''
...
def first_part(starting_position, ending_position, covered_by_blizzard_map, starting_time=0, print_result=True):
    ...
    paths_considered_to_visit[0][str([starting_time%time_until_blizzards_loop,starting_position[0], starting_position[1]])] = 0
...
def goal_to_start(starting_position, ending_position, covered_by_blizzard_map, starting_time=0, print_result=True):
    '''
    This function contains mostly the same code as first_part, varying only in which movement is checked to see if the ending position has been reached (up instead of down is checked), and the order of execution of the movements to consider (higher priority to movements that go left and up)
    '''
    time_until_blizzards_loop = covered_by_blizzard_map.shape[0]
    ...
    while len(paths_to_visit) != 0:
        ...
        current_time = current_time_and_position[0]
        
        if(current_time >= minimum_arrival_minutes):
            continue
        elif np.all(up_movement[1:] == ending_position):
            if current_time < minimum_arrival_minutes:
                minimum_arrival_minutes = current_time 
        else:
            current_time_mod = current_time % time_until_blizzards_loop
...
def second_part(starting_position, ending_position, covered_by_blizzard_map, n_minutes_first_trip=0):
...
def main(file_name):    
    ...
    minimum_arrival_minutes = first_part(starting_position, ending_position, covered_by_blizzard_map)
    second_part(starting_position, ending_position, covered_by_blizzard_map, minimum_arrival_minutes)

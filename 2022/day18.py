...
PUZZLE_INPUT_FILE_NAME = "puzzleInputs/day18.txt"
PUZZLE_EXAMPLE_INPUT_FILE_NAME = "puzzleInputs/day18_example.txt"
...
def check_surface_of_boulder_block(boulder, x_base, y_base, z_base):
    surface = N_FACES_OF_A_CUBE # assume that the exposed surface is the maximum possible one. Decrease this number if it is found that it is actually not exposed
    y = y_base
    z = z_base
    for x in [x_base-1,x_base+1]:
        surface -= boulder[(x,y,z)]
...
def is_block_a_exterior_border(boulder,x,y,z):
    idxmax = np.where(boulder[:,y,z] == 1)[0]
    if (len(idxmax) == 0):
         return True
    else:
        idxmax = np.max(idxmax)

    idymax = np.where(boulder[x,:,z] == 1)[0]
    if (len(idymax) == 0):
        return True
    else:
        idymax = np.max(idymax)

    idzmax = np.where(boulder[x,y,:] == 1)[0]
    if (len(idzmax) == 0):
        return True
    else:
...
def is_block_in_an_air_bubble(boulder,x,y,z):
    ...
    for x, y, z in np.ndindex(boulder.shape):
        if boulder[x,y,z] == True:
            ...
            if(is_block_a_exterior_border(boulder,x,y,z) == False):
                if(is_block_in_an_air_bubble(boulder,x,y,z) == True):

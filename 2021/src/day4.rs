...
const PUZZLE_INPUT_FILE_NAME: &str = "puzzleInputs/day4.txt";
const PUZZLE_EXAMPLE_INPUT_FILE_NAME: &str = "puzzleInputs/day4_example.txt";
...
#[derive(Clone)]
struct BingoBoard {
    called_numbers_positions: std::collections::HashMap<u32, (usize,usize,bool)>, // map with the numbers in the bingo board as keys. Each number (key) has its row and column position in as value, and also a boolean that tells if that key has been drawn or not
...
fn first_part(numbers_to_draw: &Vec<u32>, mut bingo_boards: Vec<BingoBoard>) {
...
fn second_part(numbers_to_draw: &Vec<u32>, mut bingo_boards: Vec<BingoBoard>) {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    first_part(&numbers_to_draw, bingo_boards.clone());
    second_part(&numbers_to_draw, bingo_boards);
